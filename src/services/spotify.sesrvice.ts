...
export class SpotifyService {
  ...
  public async getTrack(id: string): Promise<TrackResponse> {
    ...
    const track = (
      await Axios.get<TrackResponse>(`https://api.spotify.com/v1/tracks/${id}`, {
        headers: {
  ...
  public async getAlbum(id: string): Promise<TrackResponse[]> {
    ...
    let tracks = (
      await Axios.get<PaginatedTracklistResponse>(`https://api.spotify.com/v1/albums/${id}/tracks`, {
        params: {
          offset: 0,
          limit: 50,
        },
    ...
    while (tracks.next != null) {
      tracks = (
        await Axios.get<PaginatedTracklistResponse>(tracks.next, {
          headers: {
            authorization: `Bearer ${token}`,
          },
        })
  ...
  public async getPlaylist(id: string): Promise<TrackResponse[]> {
    ...
    const results: TrackResponse[] = [];
    let tracks = (
      await Axios.get<PaginatedTracklistResponse>(`https://api.spotify.com/v1/playlists/${id}/tracks`, {
        params: {
          offset: 0,
          limit: 100,
        },
  ...
  private async authenticate(): Promise<string> {
    ...
    const res = await Axios.post<{ access_token: string; token_type: string; expires_in: number; scope: string }>(
